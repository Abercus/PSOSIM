...
export default class Canvas extends Component {
  ...
  updateParticles = throttle(() => {
    this.pop.runIteration(this.props.omega, this.props.phiP, this.props.phiG, this.props.speed);
  ...
  createCircleTexture(color, size) {
    // src: https://2pha.com/blog/threejs-easy-round-circular-particles/
  ...
  resetSimulation() {
    ...
    // Clear previous best for graph drwaing.
    this.prevBest = null;
    this.scene.remove(this.particleSystem);
    // Remove previous paricles
    if (this.particles) {
    ...
    this.pMaterial = new THREE.PointsMaterial({
      size: particleSize,
      map: this.CLICKABLE_DEMO ? this.createCircleTexture('#ffffff', 256) : this.createCircleTexture('#e60000', 256),
      transparent: true,
      depthWrite: false
    })
    var pZ = null;
    ...
    for (var p = 0; p < this.props.particlesNumber; p++) {
      ...
      if (this.CLICKABLE_DEMO) {
        particle.bestNumerical = euclidDistance(particle,
  ...
  initDemoMode() {
    // Set random interval to change optimization goal.
  ...
  setupVisualization() {
    // src: https://github.com/stemkoski/stemkoski.github.com/blob/master/Three.js/Graphulus-Function.html
