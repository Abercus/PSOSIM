...
function testSphereFunction(p) {
  return sphereFunction(p.x, p.y);
}
...
//  constructor(pop, optimization_goal) {
  ...
  set_optimization_goal(vector) {
    ...
    for (var i=0; i<this.population.length; i++) {
      ...
      this.population[i].bestNumerical = testSphereFunction(this.population[i]);
      this.population[i].pBest = new THREE.Vector3(vector.x, vector.y, this.population[i].bestNumerical);
  ...
  update(phiP, phiG) {
    ...
    for (var i=0; i<this.population.length; i++) {
      ...
      //console.log("HIER");
      //console.log(this.gBest, particle)
...
*/
      ...
      particle.currentNumerical = testSphereFunction(particle);
...
export default class Canvas extends Component {
    ...
    resetSimulation() {
        ...
        for (var p = 0; p < this.props.particlesNumber; p++) {
            ...
            var numericalBest =  testSphereFunction({x:pX, y:pY});
            var particle = new THREE.Vector3(pX, pY, numericalBest);
            particle.velocity = new THREE.Vector3(0,0,0);

            particle.bestNumerical = numericalBest;
    ...
    setupScene() {
        ...
        */
        ...
        setInterval(() => {
            ...
            var z = testSphereFunction({x:x, y:y});
